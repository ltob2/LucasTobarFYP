        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    double *SPN;     /* Neuron spikes (1) over time*/
    ...
    simulLen = mxGetNumberOfElements(prhs[1])/totNnrn; 
    simulLen2 =  mxGetNumberOfElements(prhs[2])/totNnrn;
    ...
    plhs[4] = mxCreateDoubleMatrix(totNnrn,simulLen, mxREAL);
    SPN = mxGetPr(plhs[4]);
    ...
    for(t=0; t<simulLen; t++) {
        ...
        for(nrn=0; nrn<totNnrn; nrn++) {
             ...
             */  
             // Number of external spikes impinging on the nrn-th excitatory neuron
                ...
                */
                ...
                aX_ext[nrn] += aX_k2;
                //aS_ext[nrn] += aS_k2;
                aS_ext[nrn] = x2eFR[nrn+totNnrn*t];
            }
            ...
            //Stim test: failed because stimulating here has no feedback effect
            //if(t>simulLen/2&&t<(0.2/Dt+simulLen/2+1)){
            //synI = aI + gI-0;//14000000;  // Synaptic current, I_tot(t)
            //}
            //else if(t>(0.3/Dt+simulLen/2)&&t<(0.5/Dt+simulLen/2+1)){
            //synI = aI + gI+0;//14000000;
           // }
           // else{
               synI = aI + gI;  
           // }
