...
%% Running Model
S_cons = cell(1,1); %Storing frequency power spectrum values
FR_E_cons = cell(1,1); %Firing rate for excitatory populations for each contrast
FR_I_cons = cell(1,1); %Firing rate for inhibitory populations for each contrast
contrasts = [0,2,4,6]; %stimulus amplitude range
    ...
    for trial = 1:500 %500 Trials
        ...
        for t = 1:14100
            ...
            for i = 1:2
                ...
                %supragranular
                r_E = r_E + (dt/2)*(-r_E +xe/(1-exp(-xe)) + sqrt(tau_E_SG)*sigma_SG*randn(1))/tau_E_SG;
                r_I = r_I + (dt/2)*(-r_I +xi/(1-exp(-xi)) + sqrt(tau_I_SG)*sigma_SG*randn(1))/tau_I_SG;
            end
