...
for cond = 1:2 %condition 1 = coupled, condition 2 = uncoupled
    ...
    for trial = 1:100
        ...
        for t = 1:14100 %millisecond time steps
            ...
            %dt = 0.5ms
            for i = 1:2
                I_net = [J_EE J_EI 0 0; J_IE J_II J_25 0; J_52 0 J_EE J_EI; 0 0 J_IE J_II]*r;
                x = I_net+I_ext;
                r = r + 0.5.*(-r + x./(1-exp(-x)) + sqrt(tau).*sigma.*randn(4,1))./tau;
            end
...
%Note: if you want to plot average firing rates, the values are stored in FR.
...
%FR representation fig for report
